...
- `gateway.auth.identityScopes`: maps a verified trusted-proxy user or Tailscale WhoIs login to server-granted operator scopes. Email keys match case-insensitively. For each WebSocket connection, OpenClaw unions the identity grant with the device-authorized scopes, then intersects the result with `x-openclaw-scopes` when that header is present. Identity grants do not change pairing records or create scope-upgrade requests. Token, password, and no-auth connections have no verified user identity and receive no grant from this map.
