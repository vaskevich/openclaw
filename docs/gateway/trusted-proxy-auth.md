...
<ParamField path="gateway.auth.identityScopes" type="record<string, string[]>">
  Operator scopes granted to each verified trusted-proxy or Tailscale identity. Email keys match case-insensitively. Unknown scope names fail config validation.
</ParamField>
...
The map key is the verified identity string extracted by trusted-proxy auth (or
the verified Tailscale WhoIs login). Email matching is case-insensitive. On
each connection, OpenClaw adds the matching identity scopes to the
device-authorized scopes. An explicit `x-openclaw-scopes` header then caps that
combined set; an absent header does not cap it, and a present-but-empty header
removes all scopes.

These grants are session-only. They do not create or update device pairing
records, and they do not trigger scope-upgrade prompts. Token, password, and
no-auth connections do not carry a verified user identity and never receive an
identity grant.
...
- Device-less Control UI WebSocket sessions cannot self-declare permissions. OpenClaw clears their requested scope list to `[]`, then applies any matching server-side `identityScopes` grant after the proxy identity is verified.
