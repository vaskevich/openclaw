  ...
  return `${rendered.baseline.modules
    .map((moduleSurface) => {
      const projection = {
        ...
        entrypoint: moduleSurface.entrypoint,
        importSpecifier: moduleSurface.importSpecifier,
        exports: moduleSurface.exports.map(({ closureHash, declaration, exportName, kind }) => ({
          closureHash,
          declaration,
          exportName,
          kind,
      ...
      return `${sha256(JSON.stringify(projection))}  module/${encodeURIComponent(moduleSurface.entrypoint)}`;
    })
    .join("\n")}\n`;
}
