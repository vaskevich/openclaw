...
export const CORE_FIELD_HELP: Record<string, string> = {
  ...
  "gateway.auth.identityScopes":
    "Maps verified trusted-proxy or Tailscale user identities to server-granted operator scopes. Grants are added to device scopes for that connection, then narrowed by an explicit x-openclaw-scopes header when present. Email identity matching is case-insensitive.",
